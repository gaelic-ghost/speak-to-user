...
- playback startup is now admission-controlled in the Python runtime for both `sounddevice` and `wavbuffer`: the service can defer first audio until buffered chunk count and buffered audio lead look safe enough against the observed synth deficit, and the stderr log records both deferred and admitted startup decisions
